...
fun TermsAndConditionsScreen(onAccepted: () -> Unit) {
    ...
    // enableEdgeToEdge() lets content draw under the status and navigation bars; pad by the
    // system-bar insets so the title clears the status bar and the button clears the nav bar
    // (including 3-button navigation, which is taller than gesture navigation).
    Column(
        modifier = Modifier
            .fillMaxSize()
